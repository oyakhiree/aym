    ...
    <div class="bg-white rounded-2xl border border-secondary-100/80 shadow-sm overflow-hidden flex flex-col h-full">
        ...
        <div class="p-5 border-b border-secondary-100/80 flex items-center justify-between bg-gradient-to-r from-indigo-50/50 to-purple-50/30">
            <div class="flex items-center">
                ...
                <div>
                    <h3 class="font-bold text-secondary-900 text-lg tracking-tight">Exam Portal</h3>
                    <p class="text-xs text-secondary-500">Assessment management</p>
                </div>
            </div>
            <span 
                v-if="pendingExams > 0" 
        ...
        >
            ...
            <h4 class="text-secondary-900 font-semibold mb-1">All Caught Up!</h4>
            <p class="text-sm text-secondary-500 mb-5 max-w-[200px]">No pending assessments or scheduled exams.</p>
        ...
        <!-- Exam List -->
        <div v-else class="flex-1 overflow-y-auto divide-y divide-secondary-50">
            <div 
            ...
            >
                ...
                <div v-if="exam.status === 'Ready'" class="text-right">
             ...
             <!-- Pending State -->
             <div v-if="pendingExams > 0 && upcomingExams.length === 0" class="p-8 text-center">
                <div class="inline-flex items-center justify-center p-3 bg-amber-100 rounded-2xl mb-4">
                     <Clock class="w-6 h-6 text-amber-600" />
                </div>
                <h4 class="text-secondary-900 font-semibold mb-1">Awaiting Approval</h4>
                <p class="text-sm text-secondary-500">Your exam request is pending Area Coordinator review.</p>
             </div>
        </div>
        
         <!-- Footer -->
        <div v-if="upcomingExams.length > 0" class="p-4 bg-secondary-50/50 border-t border-secondary-100/80">
