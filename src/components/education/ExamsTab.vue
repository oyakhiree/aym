...
<template>
    <div class="space-y-6">
        ...
        <div class="bg-blue-50/50 border border-blue-100/60 rounded-2xl p-5 flex gap-4 text-blue-800 text-sm shadow-sm">
            ...
            <div>
                <h4 class="font-bold mb-1 text-blue-900">Final Exam Eligibility</h4>
                <p class="leading-relaxed text-blue-700/80">Students must achieve a Readiness Score of <strong class="text-blue-900">{{ READINESS_THRESHOLD }}% or higher</strong> to take the final exam. This score is calculated based on attendance (40%) and assignment grades (60%).</p>
            </div>
        </div>
            ...
            >
                <div class="flex justify-between items-start mb-5">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-bold text-secondary-900 leading-tight group-hover:text-primary-700 transition-colors">{{ student.name }}</h3>
                            <p class="text-xs text-secondary-500 font-medium">ID: {{ student.id }}</p>
                        </div>
                    </div>
