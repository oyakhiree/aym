  ...
  <div class="space-y-6 animate-in fade-in pb-20 md:pb-0">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white p-4 rounded-2xl border border-secondary-100 shadow-sm flex items-center gap-4">
            ...
            <div>
                <p class="text-xs font-semibold text-secondary-500 uppercase tracking-wider">Active Classes</p>
                <p class="text-2xl font-bold text-secondary-900">{{ store.stats.activeCount }}</p>
            </div>
        </div>
        <div class="bg-white p-4 rounded-2xl border border-secondary-100 shadow-sm flex items-center gap-4">
            ...
            <div>
                <p class="text-xs font-semibold text-secondary-500 uppercase tracking-wider">Enrolled Students</p>
                <p class="text-2xl font-bold text-secondary-900">{{ store.stats.totalStudents }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-1 rounded-2xl border border-secondary-100 sm:p-2 sticky top-20 z-10 shadow-sm">
        <div class="flex p-1 bg-secondary-100/50 rounded-xl w-full sm:w-auto">
            ...
            >
                <BookOpen v-if="tab === 'Progressive'" class="w-4 h-4" />
    ...
    <!-- Grid -->
    <div v-if="filteredClasses.length > 0" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 align-top">
        <ClassCard 
    ...
    <!-- Empty State -->
    <div v-else class="text-center py-16 bg-white rounded-3xl border border-secondary-100 border-dashed">
        ...
        <h3 class="text-lg font-bold text-secondary-900">No classes found</h3>
        ...
        <BaseButton v-if="!searchQuery" class="mt-6" @click="isCreateModalOpen = true">
            Start New Class
        </BaseButton>
    </div>

    <CreateClassModal :isOpen="isCreateModalOpen" @close="isCreateModalOpen = false" />
