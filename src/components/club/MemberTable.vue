    ...
    <table class="min-w-full">
      <thead>
        <tr class="border-b border-secondary-100 bg-secondary-50/50">
          <th
            scope="col"
            class="px-8 py-4 text-left text-xs font-semibold text-secondary-400 uppercase tracking-wider"
          >
            Member
          </th>
          <th
            scope="col"
            class="px-6 py-4 text-left text-xs font-semibold text-secondary-400 uppercase tracking-wider"
          >
            Class
          </th>
          <th
      ...
      <tbody class="divide-y divide-secondary-50">
        <tr
          ...
          :key="member.id"
          class="hover:bg-secondary-50/50 transition-colors duration-150 group"
        >
          <!-- Name & Avatar -->
          <td class="px-8 py-4 whitespace-nowrap">
            <div class="flex items-center gap-4">
              <div class="flex-shrink-0 h-10 w-10">
                <div class="h-10 w-10 rounded-xl bg-primary-50 flex items-center justify-center text-primary-600 font-semibold text-sm border border-primary-100">
                  {{ member.firstName[0] }}{{ member.lastName[0] }}
                </div>
              </div>
              <div>
                <div class="text-sm font-semibold text-secondary-900 group-hover:text-primary-600 transition-colors">
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-secondary-100 text-secondary-700 border border-secondary-200">
              {{ member.class }}
            </span>
          </td>
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            ...
            >
              <span 
                ...
                :class="[member.status === 'Active' ? 'translate-x-4' : 'translate-x-0']"
              />
            </button>
          </td>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right">
            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200 transform translate-x-2 group-hover:translate-x-0">
              ...
              <button 
                class="p-2 text-secondary-400 hover:text-red-500 hover:bg-white hover:shadow-sm rounded-lg transition-all border border-transparent hover:border-secondary-100"
                title="Delete Member"
